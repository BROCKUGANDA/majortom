# 01 — Master Spec (SPEC.md §1–§10 + Annexes)

<aside>
📄

This page is the source of `SPEC.md`. Copy it into the repo root verbatim. The Bob prompt pack (page 02) references these section numbers, so **fill sections, never renumber them**.
...
`--ignore-scripts` on install is deliberate: lifecycle scripts are the supply-chain risk surface (see page 03 §2.4). Where a fixture genuinely needs scripts, it must be opted in explicitly and logged.
...
**TO FILL (Opus, from Bob 2.0 documentation). Do not invent syntax here — a wrong config is worse than an empty section.**

</aside>

Required from the docs: (1) where agent instructions live and their file format; (2) how a subagent is declared, and whether a subagent can be given a restricted tool set; (3) how parallel tasks are launched and joined, and the concurrency ceiling; (4) how document understanding is invoked on an attached file and what it returns; (5) how tool allowlists and denylists are expressed; (6) session, context, and token limits per subagent; (7) whether subagent outputs are structured or free text; (8) resume and retry semantics if a task fails.

**Fallback (must be specified either way):** if Bob lacks native subagent primitives with tool restriction, the dispatcher runs fixers as concurrent in-process tasks, each with (a) its own contract prompt from §7, (b) a filesystem facade that physically rejects paths outside its queue, and (c) its own log file. The architecture must not depend on which path is used — and the pitch says so out loud.

## Annex B — Express 4 to 5 breaking-change catalog (draft)

<aside>
🔎

**TO VERIFY (Opus): check every row against the official Express 5 migration guide and changelog, correct the wording, add exact section titles and locators, drop anything unsupported.** The confidence column reflects pre-verification belief, not evidence.
