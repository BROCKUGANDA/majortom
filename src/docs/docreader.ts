...
const INJECTION_PATTERNS: Array<{ re: RegExp; label: string }> = [
  {
    re: /\b(ignore|disregard|forget)\s+(all\s+)?(previous|prior|above|earlier)\s+(instruction|instructions|prompt|prompts|rules?)/i,
    label: "instruction-override",
  },
  {
    re: /\b(ai|agent|assistant|llm|model|tool|bot|claude|chatgpt|gpt)\b\s*[:,]\s*(you\s+)?(must|should|shall|need to|have to|are required to)/i,
    label: "addressed-to-tool",
  },
  {
    re: /\b(delete|remove|wipe|drop)\s+all\s+(the\s+)?(files?|repo|repository|directory|source)/i,
    label: "destructive-directive",
  },
  { re: /\boutput\s+(the\s+)?(system|initial)\s+prompt\b/i, label: "prompt-exfiltration" },
  {
    re: /\byou\s+(must|should|shall|need to)\s+(now\s+)?(apply|write|commit|push|merge|run|execute)\b/i,
...
const SHAPES: ShapeRule[] = [
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports the app.del() function. Use app.delete() to register HTTP DELETE routes.",
      detect: { regex: ["\\.del\\s*\\("], astQuery: null, filesGlob: ["**/*.{ts,js,mjs,cjs}"] },
      fix: {
        strategy: "codemod",
        instruction:
          "Replace every app.del(path, handler) call with app.delete(path, handler) in this file.",
        example: {
          before: "app.del('/user/:id', handler)",
          after: "app.delete('/user/:id', handler)",
        },
      },
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports the app.param(fn) signature. Rewrite it as named param middleware.",
      detect: {
        regex: ["app\\.param\\(\\s*function"],
        astQuery: null,
      ...
      fix: {
        ...
        instruction:
          "Rewrite app.param(fn) as a named parameter middleware registered with app.param(name, fn).",
        example: {
          before: "app.param(function (name, value) {})",
          after: "app.param('user', loadUser)",
        },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 removed req.acceptsCharset(), req.acceptsEncoding() and req.acceptsLanguage(). Use the pluralized forms.",
      detect: {
      ...
      fix: {
        strategy: "codemod",
        instruction:
          "Replace req.acceptsCharset() with req.acceptsCharsets(), req.acceptsEncoding() with req.acceptsEncodings(), and req.acceptsLanguage() with req.acceptsLanguages().",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 removed req.param(name). Read the specific name from req.params, req.body, or req.query instead.",
      detect: {
        regex: ["req\\.param\\s*\\("],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      ...
      fix: {
        strategy: "guided-edit",
        instruction:
          "Replace req.param('name') with req.params.name, req.body.name, or req.query.name according to where the value is submitted.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports the res.json(obj, status) signature. Set the status first, then chain to res.json().",
      detect: {
        regex: ["res\\.json\\([^)]+,\\s*\\d{3}\\s*\\)"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      },
      fix: {
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports the res.jsonp(obj, status) signature. Set the status first, then chain to res.jsonp().",
      detect: {
        regex: ["res\\.jsonp\\([^)]+,\\s*\\d{3}"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      },
      fix: {
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports res.redirect(url, status). The new signature is res.redirect(status, url).",
      detect: {
        regex: ["res\\.redirect\\([^)]+,\\s*\\d{3}\\s*\\)"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      },
      fix: {
        strategy: "codemod",
        instruction:
          "Swap the arguments of res.redirect so the status code comes first: res.redirect(status, url).",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports the magic string 'back' in res.redirect() and res.location(). Use req.get('Referrer') with a '/' fallback.",
      detect: {
        regex: ["redirect\\(\\s*['\"]back['\"]"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      },
      fix: {
        strategy: "guided-edit",
        instruction:
          "Replace res.redirect('back') with res.redirect(req.get('Referrer') || '/') and res.location('back') with res.location(req.get('Referrer') || '/').",
        example: {
          before: "res.redirect('back')",
          after: "res.redirect(req.get('Referrer') || '/')",
        },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation: "None.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports the res.send(obj, status) signature. Set the status first, then chain to res.send().",
      detect: {
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 no longer supports res.send(status) where status is a number. Use res.sendStatus(statusCode) instead.",
      detect: {
        regex: ["res\\.send\\(\\s*\\d{3}\\s*\\)"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      },
      fix: {
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      fix: {
        ...
        example: {
          before: "res.sendfile('/path/to/file')",
          after: "res.sendFile('/path/to/file')",
        },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation: "None.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 changed path route matching: the wildcard * must be named, the optional character ? is no longer supported, and regexp characters are not supported.",
      detect: {
      ...
      fix: {
        strategy: "guided-edit",
        instruction:
          "Replace an optional route parameter with the brace form: /:file.:ext? becomes /:file{.:ext}.",
        example: { before: "'/:file.:ext?'", after: "'/:file{.:ext}'" },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation:
        "Routes with optional or wildcard segments may need their test URLs updated.",
      confidence: 0.85,
      requiresHumanReview: false,
    }),
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 requires every route wildcard to be named. Replace the bare /* wildcard with a named wildcard such as /*splat.",
      detect: {
        regex: ["/\\*[^a-zA-Z]", "/\\*$"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      ...
      fix: {
        strategy: "codemod",
        instruction:
          "Replace the bare /* wildcard in the route path with a named wildcard such as /*splat.",
        example: { before: "router.get('/*', handler)", after: "router.get('/*splat', handler)" },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation:
        "Routes using a bare wildcard may need their test URLs updated to the named form.",
      confidence: 0.9,
      requiresHumanReview: false,
    }),
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 does not support regexp characters in route strings. Replace a regexp route with an array of explicit paths or with named params.",
      detect: {
        regex: ["['\"][^'\"]*[(+[][^'\"]*['\"]"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      ...
      fix: {
        strategy: "guided-edit",
        instruction:
          "Replace a route string containing regexp characters with an array of explicit paths, or with named parameters.",
        example: {
          before: "app.get('/[discussion|page]/:slug', h)",
          after: "app.get(['/discussion/:slug', '/page/:slug'], h)",
        },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation: "None.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 forwards rejected promises from middleware and handlers to the error-handling middleware, so async handlers no longer need an explicit catch.",
      detect: {
        regex: ["\\.catch\\(next\\)"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      ...
      fix: {
        ...
        instruction:
          "Verify the error handler signature and that async handlers rely on automatic promise forwarding. This is a behavioural judgement, not a mechanical rewrite.",
        example: null,
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 makes the extended option false by default in express.urlencoded(). Pass it explicitly if extended parsing is required.",
      detect: {
        regex: ["urlencoded\\(\\s*\\)"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      ...
      fix: {
        strategy: "guided-edit",
        instruction:
          "Pass extended explicitly to express.urlencoded(), choosing true only if the app relies on nested body parsing.",
        example: {
          before: "app.use(express.urlencoded())",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 returns undefined from req.body when the body has not been parsed, where Express 4 returned {} by default.",
      detect: { regex: ["req\\.body"], astQuery: null, filesGlob: ["**/*.{ts,js,mjs,cjs}"] },
      fix: {
        strategy: "guided-edit",
        instruction:
          "Mount express.json() or express.urlencoded() before any handler that reads req.body, or guard the access with a default value.",
        example: { before: "const data = req.body", after: "const data = req.body ?? {}" },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation:
        "POST tests that relied on req.body defaulting to {} may need a body parser mounted.",
      confidence: 0.8,
      requiresHumanReview: false,
    }),
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 only accepts integers from 100 to 999 in res.status() and errors on a non-integer status code.",
      detect: {
        regex: ["res\\.status\\(\\s*[^1-9\\d]"],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      },
      fix: {
        strategy: "manual-only",
        instruction:
          "Audit every res.status() argument and replace any value outside 100-999 or any non-integer with a valid status code.",
        example: null,
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation: "None.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      fix: {
        ...
        example: {
          before: "res.clearCookie('sid', { maxAge: 0 })",
          after: "res.clearCookie('sid')",
        },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
      testExpectation: "None.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 requires a newer minimum Node.js version. The engines field and any CI node version must be bumped to match.",
      detect: {
      ...
      fix: {
        strategy: "codemod",
        instruction:
          "Set the engines.node range in package.json to the minimum required by the guide, and update the CI node version to match.",
  ...
  },
  {
    ...
    build: (s) => ({
      ...
      summary:
        "Express 5 changed the default query parser, so nested req.query object access may no longer be populated the same way.",
      detect: {
        regex: ["req\\.query\\s*\\["],
        astQuery: null,
        filesGlob: ["**/*.{ts,js,mjs,cjs}"],
      ...
      fix: {
        strategy: "guided-edit",
        instruction:
          "Set the query parser explicitly with app.set('query parser', ...) or adapt the code to the simple parser's flat result.",
        example: {
          before: "const f = req.query.filters.category",
          after: "app.set('query parser', 'extended')",
        },
      },
      citation: { docId: "", sectionTitle: s.title, locator: s.locator },
